...
class CheckoutTests(TestCase):
    ...
    def test_order_history_view(self):
        ...
        response = self.client.get(
            reverse('profiles:order_history', args=[self.order.order_number])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_payment_success_nonexistent_order(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.get(
            reverse('checkout:payment_success', args=['NONEXISTENT'])
        )
    ...
    def test_payment_success_flow(self, mock_payment_retrieve):
        ...
        mock_payment_retrieve.return_value = payment_intent
        ...
        response = self.client.get(
            reverse('checkout:payment_success', args=[order.order_number])
        )
        ...
        self.assertFalse(
            Cart.objects.filter(cart_id=session.session_key).exists()
        )
